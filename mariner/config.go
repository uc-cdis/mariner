...
var (
	...
	WORKFLOW_VOLUMES = []string{ENGINE_WORKSPACE_VOLUME, COMMONS_DATA_VOLUME, USER_DATA_VOLUME}
)
...
const (
	...
	// volume names
	ENGINE_WORKSPACE_VOLUME = "engine-workspace"
	COMMONS_DATA_VOLUME     = "commons-data"
	USER_DATA_VOLUME        = "user-data"
	CONFIG_VOLUME           = "mariner-config"
)
...
func getVolumeMounts(component string) (volumeMounts []k8sv1.VolumeMount) {
	for _, v := range WORKFLOW_VOLUMES {
		volumeMount := getNamedVolumeMount(v)
		switch component {
		case TASK, ENGINE:
	...
	if component == ENGINE {
		configVol := getNamedVolumeMount(CONFIG_VOLUME)
...
func getNamedVolumeMount(name string) *k8sv1.VolumeMount {
	...
	if name == ENGINE_WORKSPACE_VOLUME {
